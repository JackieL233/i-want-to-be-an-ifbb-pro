...
- `Metrics Flow Coach / 身体数据流程教练`：`Metrics` 第一屏只突出今天数据下一步，例如连接 Health Connect、同步今天身体数据、补体重/腰围、拍体型照片或查看证据；用户先补齐 AI 复盘最需要的数据，再展开细节。
- `Metrics detail layers / 数据细节层`：Health Connect、Recovery Guidance、Conditioning & Hydration、Physique Measurement Summary、Physique Photo Check-in、Photo Evidence、Trend Overview 和完整身体/恢复输入字段默认收在数据细节层里，需要检查证据或手动补录时再展开。
- `AI Review Flow Coach / AI 复盘流程教练`：`AI Coach` 第一屏只判断现在是否应该先设置 API、补训练/饮食/身体数据、添加照片，还是直接 `Run daily review / 运行每日复盘`；用户不需要先理解所有 AI 参数和数据地图。
- `AI detail layers / AI 细节层`：API 设置、照片用途、额外提示词、Photo Evidence、AI Review Action Queue、Saved AI Reviews 和 AI Data Map 默认收在细节层里，需要上传照片、改模型、做专项模式分析或检查证据时再展开。
...
- Plan Flow Coach：把 `Template -> weekly plan -> Apply today` 变成一个下一步动作，会根据训练经验和每周训练天数推荐起始模板，并把选中的训练日转换成今天的 set-level 训练日志。
...
- Metrics Flow Coach：把 Health Connect 授权、今日同步、手动补录、体型照片和恢复证据合成一个下一步动作，让用户每天知道应该先同步数据、补关键字段还是拍进度照。
- AI Review Flow Coach：把 API 设置状态、Daily Execution Plan、训练完成度、餐食记录、身体数据、照片证据、趋势窗口和已保存复盘合成一个下一步动作，让用户知道应该先补证据还是直接运行每日复盘。
