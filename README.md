...
The app now supports Athlete Profile setup, weekly training plans, set-level workout execution, and Health Connect metric sync: define the long-term physique goal, phase, training experience, target weight/body fat, weekly training days, equipment, weak points, and constraints; build a weekly plan; apply a planned day to today's workout; record each set's weight, reps, RIR, completion state, rest time, and notes; tap a completed set to start a rest countdown; sync user-authorized body weight, body fat, lean mass, steps, sleep, resting heart rate, and calorie burn from Health Connect when Xiaomi/Mi Fitness, Huawei, scale, watch, or phone apps expose those records; then send the athlete profile, weekly plan, actual session log, meals, photos, body metrics, sleep, steps, hunger, fatigue, soreness, and stress into the AI review flow.
