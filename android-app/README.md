...
5. Use `Nutrition` to set calorie/macronutrient targets, follow Nutrition Pacing for remaining calories/macros and next-meal focus, use Next Meal Builder for the next practical protein/carbs/fat/fiber target and timing cue, use Meal Logging Coach to prefill the suggested meal, add the recommended template, attach food/menu/label photos, or open AI review, follow Meal Assembly Guide for the actual plate structure, protein anchor, carb anchor, fat control, shopping/prep cue, and food-photo logging cue, quick-add common meals from Meal Templates, check Body Composition Guidance before changing targets, log meals manually, attach a meal photo when you want food-photo analysis, and use Conditioning & Hydration to log water, sodium, caffeine, alcohol, digestion, cardio, and scale-weight noise.
