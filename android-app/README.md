...
- A global `First-run quick start` card with `FirstRunSetupCard` for the first open experience. It frames a `3-minute setup` around `Goal, split, food, health, AI key`, then gives the beginner one primary action: `Start with recommended setup`. The supporting path is `Open profile, connect health, set API` before the daily command becomes the main workflow.
...
- A `Today meal timeline` on `Nutrition`, showing `Logged meals with time, macros, and remaining targets` while keeping `Meal evidence for AI review`.
...
1. Set `Athlete Profile` in the Training plan layer so the app knows your phase, target, training schedule, equipment, weak points, and constraints.
   On first open, use `First-run quick start`: complete the 3-minute setup for goal, split, food, health, and AI key, then tap `Start with recommended setup`. The helper path is `Open profile, connect health, set API`.
